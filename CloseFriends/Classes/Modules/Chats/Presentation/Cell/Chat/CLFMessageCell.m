...
@interface CLFMessageCell() < UIGestureRecognizerDelegate>
{
...
- (void)setInitialSizes
{
    ...
    self.timeLabel.font = [UIFont fontWithName:@"AvenirNext-Regular" size:12];
...
#pragma mark -
- (void)setMessage:(id<CLFMessage>)message
{
    _message = message;
...
- (void)adjustCell
{
    ...
    if (self.message.type == CLFMessageTypeText)
    {
        self.textView.hidden = NO;
        [self adjustForTextOnly];
    }
    else if (self.message.type == CLFMessageTypePhoto)
    {
        self.mediaImageView.hidden = NO;
        [self adjustForPhotoOnly];
    }
    else if (self.message.type == CLFMessageTypeVideo)
    {
        self.mediaImageView.hidden = NO;
        [self adjustForVideoOnly];
    }
    else if (self.message.type == CLFMessageTypeAudio)
    {
        self.mediaImageView.hidden = NO;
        [self adjustForAudioOnly];
    }
    else if (self.message.type == CLFMessageTypeLocation)
    {
        self.mediaImageView.hidden = NO;
        [self adjustForLocationOnly];
    }
    else if (self.message.type == CLFMessageTypeOther)
    {
...
- (void)adjustForVideoOnly
{
    ...
    UIImageView *playButtonImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"play_button"]];
...
-(void) adjustForAudioOnly
{
    NSLog(@"adjust for audio only");
}

-(void) adjustForLocationOnly
{
    NSLog(@"adjust for location only");
}
    ...
    if (isHorizontalPan)
    {
        ...
        if (pan.state == UIGestureRecognizerStateEnded || pan.state == UIGestureRecognizerStateCancelled || pan.state == UIGestureRecognizerStateFailed)
        {
            ...
            [UIView animateWithDuration:0.25 animations:^{
                ...
                for (CLFMessageCell *cell in visibleCells)
                {
                    contentOffsetX = 0;
